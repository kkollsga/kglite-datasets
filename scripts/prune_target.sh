...
expected_target="/Volumes/EksternalHome/coding-cache/cargo-targets/kglite-datasets"
...
if [ "$free_kib" -lt "$clean_below_kib" ] || [ "$size_kib" -ge "$clean_above_kib" ]; then
  echo "pruning kglite-datasets cargo cache (free=${free_kib} KiB, size=${size_kib} KiB)"
  cargo clean --target-dir "$actual_target"
  write_cache_tag
else
  echo "kglite-datasets cargo cache within bounds (free=${free_kib} KiB, size=${size_kib} KiB)"
fi
