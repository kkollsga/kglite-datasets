...
0.15.8, 0.16.5, 0.16.6, 0.16.7, 0.16.9, 0.16.12, 0.16.13, 0.16.15, and
0.16.16 (last checked 2026-08-31) — the digest is
...
0.16.6, 0.16.7, 0.16.9, 0.16.12, 0.16.13, 0.16.15, and 0.16.16 (0.16.6's
per-section ``.kgl`` CRCs change bytes, not topology).
...
def test_save_reload_round_trip(tmp_path: Path) -> None:
    ...
    kwarg, taking whatever mode the checkpoint serves by default. Since kglite
    0.16.15 an explicit ``storage="memory"``/``"mapped"`` on ``load`` is an
    *override* rather than an error (``"disk"`` is still refused structurally —
    a ``.kgl`` is a file, a disk graph is a directory); the cache-hit contract
    under test is the bare-``load`` path the wrapper actually calls.
    """
    g = _build_memory_graph(tmp_path)
